<template>
	<view>
		...
		<!-- <view class="article-content ql-snow">
			<rich-text class="ql-editor" :nodes="htmlNodes"></rich-text>
		</view> -->
		<view class="article-content ql-snow">
			<view  class="ql-editor" v-html="htmlNodes"></view>
		</view>
		...
		methods: {
			getDetail(id) {
				this.$api.getArticleDetail(id).then((res) => {
					...
					if (err_code === 0) {
						this.htmlNodes = data.article_content;
						this.banner.cover = data.cover;
						this.banner.title = data.article_name;
						this.banner.published_at = data.article_time;
						this.banner.author_name = 'neo';
					}
				})
				// uni.request({
